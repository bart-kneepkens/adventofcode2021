...
//  Advent Of Code 2021
//
//  Created by Bart Kneepken on 09/12/2021.
//
...
struct Day8 {
    static var part1: Int {
        ...
        return outputs.filter({ $0.count == 3 || $0.count == 2 || $0.count == 4 || $0.count == 7 }).count
    }
    
    static var part2: Int {
        let outputs = shortInput
            .components(separatedBy: .newlines)
            .map({ $0.components(separatedBy: " | ")[1]})
            .flatMap({ $0.components(separatedBy: .whitespaces )})
        
        let values: [Set<String.Element>: Int] = [
            Set("cagedb"): 0,
            Set("ab"): 1,
            Set("gcdfa"): 2,
            Set("fbcad"): 3,
        ...
        let numbers: [Int] = outputs.map { output in
            let set = Set(output)
